...
async function get(req, res) {
    let obj = await prisma.agencyRatingInterface.findMany({
        where: {
            rate: { equals: req.query.rate },
...
async function remove(req, res) {
    let obj = await prisma.agencyRatingInterface.delete({ where: { id: req.query.id } });
    res.status(200).json(obj);
}
